...
import { 
  ShoppingBag, Clock, MapPin, Package, 
  CheckCircle2, Truck, XCircle, Navigation
} from 'lucide-react';
import { Link } from 'react-router-dom';
import { useSocket } from '../context/SocketContext';
import toast from 'react-hot-toast';
import LiveTracking from '../components/order/LiveTracking';
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        ...
        ></motion.div>
        <p className="mt-4 text-gray-500 italic font-medium">Fetching your delicious orders...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 px-4">
        <motion.div 
          ...
          className="bg-white p-12 rounded-[40px] shadow-xl border border-gray-50 flex flex-col items-center"
        >
          ...
          {orders.map((order, index) => {
            ...
            return (
              <motion.div 
                ...
                className="bg-white dark:bg-gray-800 rounded-2xl sm:rounded-[32px] shadow-xl border border-gray-50 dark:border-gray-700 overflow-hidden group"
              >
                {/* Order Header */}
                      ...
                      <div className="flex gap-3">
                        {['accepted', 'preparing', 'ready', 'out_for_delivery'].includes(order.status) && (
                          <button 
                            ...
                            className="bg-primary text-white px-4 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-primary-dark transition-all shadow-lg shadow-primary/20 flex items-center gap-2"
                          >
